"""Stage B3 manifest, preflight, and frozen plan helpers."""
...
STAGE_B3_OBJECTIVE = "proxy_anchor"
REQUIRED_STAGE_B3_RATIO = 0.5
REQUIRED_STAGE_B3_SEEDS = (20260506, 20260507, 20260508)
STAGE_B3_GLM_EXCLUSION_REASON = (
    "answer format incompatible with frozen yes/no population rule"
)


def load_stage_b3_panel(full_cache_root: Path | str) -> StageBPanelManifest:
    """Load the Stage B3 panel from the unified full-cache manifest only."""

    return load_stage_b_panel_manifest(full_cache_root)


def validate_stage_b3_plan(
    *,
    ratio: float,
    seeds: Sequence[int],
    objective: str,
    ...
    """Validate the frozen Stage B3 experiment surface."""

    if str(objective) != STAGE_B3_OBJECTIVE:
        raise ValueError("Stage B3 trains Proxy Anchor only; objective must be proxy_anchor")
    if str(encoder_family) != STAGE_B_ENCODER_FAMILY:
        raise ValueError(f"Stage B3 encoder must be {STAGE_B_ENCODER_FAMILY}")
    if round(float(ratio), 6) != REQUIRED_STAGE_B3_RATIO:
        raise ValueError("Stage B3 negative-budget ratio must be fixed to 0.5")
    seed_values = [int(seed) for seed in seeds]
    if tuple(seed_values) != REQUIRED_STAGE_B3_SEEDS:
        raise ValueError(
            "Stage B3 seeds must be fixed to "
            + ", ".join(str(seed) for seed in REQUIRED_STAGE_B3_SEEDS)
        )
    return {
...
def build_stage_b3_preflight(
    ...
    *,
    ...
    """Build a compact Stage B3 preflight status payload."""
    ...
    if missing_exclusion_rows:
        raise ValueError(
            "Stage B3 excluded model(s) are not in the panel: "
    ...
    return {
        ...
        "fixed_objective": STAGE_B3_OBJECTIVE,
        "fixed_negative_budget_ratio": REQUIRED_STAGE_B3_RATIO,
        "fixed_seeds": list(REQUIRED_STAGE_B3_SEEDS),
