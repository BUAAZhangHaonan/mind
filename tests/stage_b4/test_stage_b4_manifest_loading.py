...
def test_stage_b4_uses_unified_manifest_and_keeps_glm_in_panel_but_excluded(tmp_path: Path) -> None:
    load_stage_b4_panel = stage_b4_attr("stage_b4_manifest", "load_stage_b4_panel")
    build_stage_b4_preflight = stage_b4_attr("stage_b4_manifest", "build_stage_b4_preflight")
    ...
    panel = load_stage_b4_panel(full_cache_root)
    ...
    assert preflight["excluded_models"] == {GLM_MODEL_ALIAS: STAGE_B4_GLM_EXCLUSION_REASON}
    ...
    assert preflight["stage_c_started"] is False
    assert preflight["detector_selected"] is False
    assert preflight["cache_root_readiness"] == "ready"


def test_stage_b4_fixed_plan_rejects_non_frozen_values() -> None:
    validate_stage_b4_plan = stage_b4_attr("stage_b4_manifest", "validate_stage_b4_plan")
