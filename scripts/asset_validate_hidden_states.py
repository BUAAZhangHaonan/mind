...
def run_validation(*, output_root: Path, smoke_cache_root: Path) -> int:
    ...
    for smoke_row in smoke_rows:
        ...
        try:
            if not _is_relative_to(shard_path.resolve(), smoke_cache_root.resolve()):
                raise ValueError(f"shard path is outside --smoke-cache-root: {shard_path}")
            ...
            determinism = checksums.get("determinism", {}).get(key)
            if status == "verified" and not determinism:
                status = AssetStatus.FAILED_VALIDATION.value
                reason = "determinism metadata missing"
...
def aggregate_model_statuses(rows: Sequence[Mapping[str, object]]) -> tuple[dict[str, str], dict[str, str]]:
    ...
    for alias in REQUIRED_MODEL_ALIASES:
        ...
        if not alias_rows:
            ...
            continue
        if all(row.get("status") == AssetStatus.VERIFIED.value for row in alias_rows):
...
def write_markdown_report(path: Path, summary: Mapping[str, object]) -> None:
    ...
    for group_key in ("blocked_reasons", "unsupported_reasons", "failed_reasons"):
