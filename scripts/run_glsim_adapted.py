#!/usr/bin/env python3
"""Run the GLSim-adapted queried-object path on cached pre-generation readouts."""
...
from mind.comparators.glsim_adapted import resolve_readout_total_layers, summarize_glsim_results
...
def build_output_paths(output_root: Path, experiment_name: str) -> dict[str, Path]:
    ...
    return {
        "metrics": root / "glsim_adapted.json",
        "results": root / "glsim_adapted_results.csv",
        "selection": root / "glsim_adapted_selection.csv",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    default_layers = resolve_glsim_adapted_layer_indices(total_layers)
    ...
    contexts = build_glsim_adapted_object_token_contexts(
        readout_entries,
        wrapper=wrapper,
        processor=processor,
    )
    score_frame = build_glsim_adapted_score_frame(
        readout_entries,
        contexts=contexts,
    ...
    if args.split_strategy == "object_heldout":
        if args.reference_root is None or not args.model_name:
            raise ValueError("--reference-root and --model-name are required for object_heldout GLSim-adapted.")
        ...
        print(
            "[run_glsim_adapted] object_heldout support "
    ...
    metrics, results, selection = evaluate_glsim_adapted_nested(
        score_frame,
